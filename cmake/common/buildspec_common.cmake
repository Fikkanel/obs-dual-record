...
function(_setup_obs_studio)
  ...
  if(OS_WINDOWS)
    ...
    set(_cmake_extra "-DCMAKE_SYSTEM_VERSION=${CMAKE_SYSTEM_VERSION} -DCMAKE_ENABLE_SCRIPTING=OFF -DCMAKE_TLS_VERIFY=0")
  elseif(OS_MACOS)
  ...
  list(JOIN CMAKE_PREFIX_PATH "\\;" _prefix_arg)
  message(STATUS "Configure ${label} (${arch})")
  execute_process(
    COMMAND
      "${CMAKE_COMMAND}" -S "${dependencies_dir}/${_obs_destination}" -B
      "${dependencies_dir}/${_obs_destination}/build_${arch}" -G ${_cmake_generator} "${_cmake_arch}"
      -DOBS_CMAKE_VERSION:STRING=3.0.0 -DENABLE_PLUGINS:BOOL=OFF -DENABLE_FRONTEND:BOOL=OFF -DENABLE_GRAPHICS_HOOKS:BOOL=OFF
      -DOBS_VERSION_OVERRIDE:STRING=${_obs_version} "-DCMAKE_PREFIX_PATH=${_prefix_arg}" ${_is_fresh}
      ${_cmake_extra}
  ...
  execute_process(
    COMMAND "${CMAKE_COMMAND}" --build build_${arch} --target libobs --target obs-frontend-api --config Debug --parallel
  ...
  execute_process(
    COMMAND "${CMAKE_COMMAND}" --build build_${arch} --target libobs --target obs-frontend-api --config Release --parallel
    WORKING_DIRECTORY "${dependencies_dir}/${_obs_destination}"
    RESULT_VARIABLE _process_result
    COMMAND_ERROR_IS_FATAL ANY
    OUTPUT_QUIET
